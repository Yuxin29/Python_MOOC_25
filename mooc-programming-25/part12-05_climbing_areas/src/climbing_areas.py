...
def sort_by_number_of_routes(areas:list):
    def by_number(area):
        return area.routes()
    routes_by_number = sorted(areas, key = by_number)
    return routes_by_number

def sort_by_most_difficult(areas:list):
    def most_difficult(area: ClimbingArea):
        return area.hardest_route().grade
    routes_by_difficulty = sorted(areas, key = most_difficult, reverse = True)
    return routes_by_difficulty
